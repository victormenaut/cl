...
class LogRegModel:
    ...
    '''def test_isabel(self):
        feats = [self.test_distribution, self.test_prev_pos, self.test_target_pos,
        ...
        try:
            assert int(len(test_feats)) == int(dims)
        except:
            print("Test Dist Len: " + str(len(self.test_distribution[0])))
            print("Test Prev POS: " + str(len(self.test_prev_pos[0])))
            print("Test Targ POS: " + str(len(self.test_target_pos[0])))
            print("Test Next POS: " + str(len(self.test_next_pos[0])))
            print("Test Feats: " + str(len(test_feats[0])))
            print("Dims Value: " + str(dims))
        ...
        with open('/Users/eholgate/PycharmProjects/twotter/hate_speech_model/hate_speech_preds.txt','w') as out:
...
main()
"""print("Cleaning the data.")
    clean_train, clean_test, target_dict, most_common = clean_isabel("train_set.txt",
                                                                              "train_tagged.txt",
                                                    "../hate_speech_model/hate_speech_indexed.csv",
                                                    "../hate_speech_model/hate_speech_tagged.txt")

    brown_one_hot_mapping = logreg.init_train(clean_train)
    logreg.init_test(clean_test, brown_one_hot_mapping)
    logreg.train()
    logreg.test_isabel()



def clean_isabel(train_path, train_parse_path, test_path, test_parse_path):
    print("Instantiating the Brown Cluster dictionary.")
    clusters = instantiate_clusters()
    print("Done.")
    ...
    print("Reading in the positive sentiment lexicon.")
    positive_dict = set([])
    with open('sentiment_lexicons/positive-words.txt', 'r') as f:
        for line in f:
            if line != '':
                if line[0] != ';':
                    positive_dict.add(line.strip())
    ...
    # Read in the testing input
    with open(test_path, 'r') as f:
        test_tweets_raw = f.read().split("\n")
    while '' in test_tweets_raw:
        test_tweets_raw.remove('')
    with open(test_parse_path, 'r') as f:
        test_parse_raw = f.read().split('\n')
    while '' in test_parse_raw:
        test_parse_raw.remove('')

    # Prune the test set to be only vulgar
    test_tweets = [test_tweets_raw[0]]
    test_parse = [test_parse_raw[0]]
    for i in range(1, len(test_tweets_raw)):
        spline = test_tweets_raw[i].split('\t')
        if spline[2] == '1':
            test_tweets.append(test_tweets_raw[i])
            test_parse.append(test_parse_raw[i])
    ...
    for line in range(1, len(train_tweets)):
        # Extract the right information
        line_array = train_tweets[line].split('\t')
        train_cleaned['target'].append(one_hot_target(line_array[5].lower().strip(), target_dict))
        train_cleaned['label'].append(int(line_array[12]))
        train_cleaned["most_common"].append(one_hot_most_common(line_array[5].lower().strip(),
                                                                most_common))
        train_cleaned['distribution'].append(get_distribution_vec(line_array[5].lower().strip(),
                                                            most_common))

        # Extract the parts of speech
        pos_line = train_parse[line].split()
        this_id = line_array[0]
        idx = int(this_id.split('_')[-1])
        sent_len = len(pos_line)

        train_cleaned['target_pos'].append(one_hot_pos(pos_line[idx].split('_')[-1], pos_dict))
        if idx != 0:
            train_cleaned['prev_pos'].append(one_hot_pos(pos_line[idx - 1].split('_')[-1], pos_dict))
        else:
            train_cleaned['prev_pos'].append(one_hot_pos("START", pos_dict))
        if idx != (sent_len - 1):
            train_cleaned['next_pos'].append(one_hot_pos(pos_line[idx + 1].split('_')[-1], pos_dict))
        ...
        # Get the RAW brown clusters feature
        train_cleaned['brown_context'].append(get_brown_feature_context(text, idx, clusters))
    ...
    for line in range(1, len(test_tweets)):
        # Extract the right information
        line_array = test_tweets[line].split('\t')
        test_cleaned['target'].append(one_hot_target(line_array[4].lower().strip(), target_dict))
        #test_cleaned['label'].append(int(line_array[12]))
        # test_cleaned["most_common"].append(
        #    one_hot_most_common(line_array[4].lower().strip(), most_common))
        test_cleaned['distribution'].append(get_distribution_vec(line_array[4].lower().strip(), most_common))

        # Extract the parts of speech
        pos_line = test_parse[line].split()
        this_id = line_array[0]
        idx = int(line_array[-1])
        sent_len = len(pos_line)
